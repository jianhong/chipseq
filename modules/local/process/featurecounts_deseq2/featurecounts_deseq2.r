...
################################################
################################################
opt$featurecount_file <- strsplit(opt$featurecount_file, " ")[[1]]
featurecount_file <- lapply(opt$featurecount_file, read.delim, header=TRUE, skip=1)
count.table <- do.call(cbind, lapply(featurecount_file, function(.ele) .ele[, -seq.int(6)]))
colnames(count.table) <- gsub(".featureCounts.txt","",basename(opt$featurecount_file))
rownames(count.table) <- featurecount_file[[1]][,"Geneid"]
interval.table <- featurecount_file[[1]][,1:6]
