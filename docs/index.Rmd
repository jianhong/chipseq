...
loadExecInfo <- function(exec_info, launchdir){
  ...
  data <- read.delim(exec_info)
  data <- data[, c("name", "status", "exit")]
  datatable(data) %>% formatStyle(
    'exit',
    backgroundColor = styleInterval(c(-Inf, 1, Inf), c('green', 'red'))
  )
}
tryCatch({
  loadExecInfo(exec_info, launchdir)
}, error=function(e){
...
1. usableFragments(
    * narrowPeak: all samples will be considered as narrowPeak samples
        * over 20 million, PASS;
        * over 10 million, Margin;
        * less 10 million, Failed;
    * broadPeak: 
        * over 45 million, PASS;
        * over 20 million, Margin;
        * less 20 million, Failed),
2. libraryComplexity:
    * NRF: 
      * over 0.9, PASS;
      * over 0.5, Margin;
...
3. Fraction of reads in peaks (FRiP)
    ...
    * over 1%, Margin;
    * less 1%, Failed;
4. Normalized Strand Cross-correlation coefficient (NSC)
    * over 1.1, PASS;
    * over 1.05, Margin;
    * less 1.05, Failed;
5. Relative Strand Cross-correlation coefficient (RSC)
    * over 1, PASS;
    * over 0.8, Margin;
    * less 0.8, Failed;
    
Color code: Green, PASS; Yellow, Margin; Red, Failed; Gray, N/A;

```{r, echo=FALSE}
rsc_files <- dir(".", "spp_rsc_mqc.tsv", recursive = TRUE, full.names = TRUE)
rsc <- do.call(rbind, lapply(rsc_files, read.delim, comment.char="#", header=FALSE))
colnames(rsc) <- c("ID", "RSC")
nsc_files <- dir(".", "spp_nsc_mqc.tsv", recursive = TRUE, full.names = TRUE)
nsc <- do.call(rbind, lapply(nsc_files, read.delim, comment.char="#", header=FALSE))
colnames(nsc) <- c("ID", "NSC")
preseq_logs <- dir(".", "", recursive = TRUE, full.names = TRUE)
...
FRiP <- lapply(FRiP_files, read.delim, comment.char="#", header=FALSE)
FRiP <- do.call(rbind, FRiP)
colnames(FRiP) <- c("ID", "FRiP")
...
breaks <- list("usableFragments"=c(0, 10000000, 20000000, Inf),
               NRF=c(0, .5, .9, Inf),
               PBC1=c(0, .5, .9, Inf),
               PBC2=c(0, 1, 10, Inf),
...
col_tab <- mapply(function(dat, point){
  cut(dat, point, labels=c("Failed", "Margin", "PASS"))
}, qc_table, breaks, SIMPLIFY=FALSE)
col_tab <- lapply(col_tab, as.character)
col_tab <- do.call(cbind, col_tab)
rownames(col_tab) <- rn
col_tab <- melt(col_tab)
col_tab[is.na(col_tab)] <- "NA"
ggplot(col_tab, aes(x=Var2, y=Var1, color=value)) + geom_point(size=5) + 
